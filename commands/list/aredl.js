...
module.exports = {
			...
			subcommand
				...
				.addStringOption((option) =>
					option
						...
						.addChoices(
							{ name: "Points excluding packs", value: "RawPoints" },
	...
	async autocomplete(interaction) {
		...
		if (focused.name === "level") {
			...
			return await interaction.respond(
				await levels
					.filter((level) =>
						level.name.toLowerCase().includes(focused.value.toLowerCase())
					)
					.slice(0, 25)
					.map((level) => ({
			...
			return await interaction.respond(
				await countries
					.filter((country) =>
						country.country.toLowerCase().includes(focused.value.toLowerCase())
					)
					.slice(0, 25)
					.map((country) => ({
	...
	async execute(interaction) {
		...
		if (subcommand === "level") {
			...
			if (level.verifications.length > 0) {
				...
				if (match[1]) {
					...
					let verInfoText = new TextDisplayBuilder().setContent(
						`Verified by [${verification.submitted_by.global_name
						}](https://aredl.net/profiles/${verification.submitted_by.id
						}), published by [${level.publisher.global_name
						}](https://aredl.net/profiles/${level.publisher.id})${levelCreators.length > 0
			...
			if (lbRes.error) {
				logger.log(`failed to fetch leaderboard: ${lbRes.data.message}`);
			...
			let container = new ContainerBuilder()
				...
				.addTextDisplayComponents(
					new TextDisplayBuilder().setContent(
						"## AREDL Leaderboard " + (nameFilter ? `(${nameFilter}) ` : "") + (country ? `(${country.country}) ` : ``)
					)
			...
			container
				...
				.addTextDisplayComponents(
					new TextDisplayBuilder().setContent(
						`Last updated at ${lastRefreshed.toLocaleTimeString(undefined, {
							hour: "numeric",
							minute: "2-digit",
							// exclude seconds
