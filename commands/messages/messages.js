...
module.exports = {
            ...
            subcommand
                ...
                .addStringOption((option) =>
                    option
                        .setName('name')
                        .setDescription(
                            'Internal name of the message to delete',
                        )
                        .setAutocomplete(true)
                        .setRequired(true),
                ),
    ...
    async execute(interaction) {
        ...
        if (subcommand === 'create') {
            ...
            const content =
                submittedModal.fields.getTextInputValue('contentInput');
            ...
            const messageEntry = await db.messages.findOne({
                where: { name: name, guild: interaction.guild.id },
            });
            if (!messageEntry) {
                return await interaction.reply({
            ...
            const editSubmittedModal = await interaction
                .awaitModalSubmit({ filter: editFilter, time: 60_000 })
                .catch(() => null);
