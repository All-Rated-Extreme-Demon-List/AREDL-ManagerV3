...
module.exports = {
	...
	async execute(interaction) {
		...
		const logStr = `${member.user.tag} (\`${member.id}\`) was timed out by ${interaction.user.tag
			} (\`${interaction.user.id}\`) for ${durationStr}.\nReason: \`${reason}\``;

		const timeout = await member.timeout(duration, logStr)
			.catch(() => null);
		if (!timeout) {
			return interaction.editReply(
		...
		// log timeout
		const logChannel = interaction.guild.channels.cache.get(timeoutLogsChannelID);
		if (logChannel && logChannel.isTextBased()) {
			logChannel.send(logStr).catch((e) => logger.error(`/timeout - Failed to send timeout log message: ${e}`));
		} else {
			logger.warn(
