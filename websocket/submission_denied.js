...
module.exports = {
	...
	async handle(client, data) {
		...
		const archiveEmbed = new EmbedBuilder()
			...
			.addFields(
				[
					...
					{ name: 'Record rejected by',  value: `<@${reviewerResponse.data.discord_id}>` },
		...
		staffGuild.channels.cache.get(isPlat ? platArchiveRecordsID : classicArchiveRecordsID).send({ embeds: [archiveEmbed] });
		guild.channels.cache.get(isPlat ? platRecordsID : classicRecordsID).send({ content : `<@${submitterResponse.data.discord_id}>`, embeds: [publicEmbed] });
		guild.channels.cache.get(isPlat ? platRecordsID : classicRecordsID).send({ content : `${data.video_url}` });
