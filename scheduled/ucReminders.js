...
module.exports = {
	...
	async execute() {
		...
		for (const submission of submissions) {
			...
			if (alreadyReminded.find((reminded) => reminded.id === submission.id)) {
				console.log("This reminder was already sent!")
				continue;
			}
			const updatedAt = new Date(submission.updated_at);
			// ucReminderThreshold in days
			const threshold = new Date(Date.now() - ucReminderThreshold * 24 * 60 * 60 * 1000);
			...
			messageLines.push(
				`[This submission](${`<https://staff.aredl.net/dashboard/submissions/${submission.id}>`}) has been Under Consideration for ${diff} days (since ${
